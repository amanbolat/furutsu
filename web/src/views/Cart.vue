<template lang="pug">
  v-container(fluid)
    ...
    v-row(v-if="cart" dense)
      v-col.mt-4
        span.title Products without discounts
      v-col(
        cols="12"
      ...
      v-col.mt-4
        span.title Products with discounts
      v-col(
        cols="12"
                ...
                v-list-item
                  ...
                  v-list-item-content
                    ...
                    v-list-item-subtitle
                      | Price: {{ sumFromCents(item.product.price).toFixed(2)  }}$
                    v-list-item-subtitle
                      .red--text.font-weight-bold Discount: {{ item.discount }}%
              v-spacer
              v-col.d-flex.justify-end.align-end.flex-column
                span.body-1 Total:
              ...
              v-col(cols="6")
                v-list
                  template(v-for="(item, idx) in set")
                    v-subheader.red--text.font-weight-bold(v-if="idx === 0") {{ item.discount }}% set discount
                    v-list-item(:key="idx")
                      v-list-item-avatar
                        v-img(:src="`/img/${item.product.name}.jpg`")
                      v-list-item-content
